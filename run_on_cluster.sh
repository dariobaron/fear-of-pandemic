...
if [ -z ${SGE_TASK_ID+x} ]; then 
	...
	index_reaction=$(( (${SGE_TASK_ID}-1)/(n_d*n_f) ))
	index_distribution=$(( (${SGE_TASK_ID}-1)/n_f%n_d ))
	index_feedback=$(( (${SGE_TASK_ID}-1)%n_f ))
	echo "src/simulation.x 'input/bianconi-barabasi_10000_10/' ${REACTIONTYPE[index_reaction]} ${FEARDISTRIBUTION[index_distribution]} ${FEEDBACKTYPE[index_feedback]}"
	src/simulation.x 'input/bianconi-barabasi_10000_10/' ${REACTIONTYPE[index_reaction]} ${FEARDISTRIBUTION[index_distribution]} ${FEEDBACKTYPE[index_feedback]}
fi
